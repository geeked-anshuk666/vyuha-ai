                ...
                <div key={node.node_name} className={`flex items-center justify-between p-4 rounded-xl transition-all duration-500 ${
                  ...
                  <div className="flex items-center gap-8">
                    <div className="text-right">
                      <div className={`text-[11px] font-black tracking-widest uppercase mb-1 ${node.state === "HEALTHY" ? "text-emerald-400" : "text-rose-500"}`}>
                        {node.state === "HEALTHY" ? "Online" : "Fatal Error"}
                      </div>
                      {node.response_time_ms && (
                        <div className="flex items-baseline justify-end gap-0.5">
                          <span className="text-2xl font-black font-mono tracking-tight text-vyuha-primary/80 leading-none group-hover:text-vyuha-primary transition-colors">
                           {node.response_time_ms.toFixed(0)}
                          </span>
                          <span className="text-[10px] font-bold text-vyuha-primary/30 uppercase tracking-tighter">ms</span>
                        </div>
                      )}
                    </div>
                    <div className={`w-3.5 h-3.5 rounded-full border-2 ${node.state === "HEALTHY" ? "bg-emerald-500 border-emerald-400/50 animate-pulse shadow-[0_0_20px_#10b981]" : "bg-rose-600 border-rose-400/50 shadow-[0_0_25px_#f43f5e] animate-pulse"}`} />
                  </div>
                </div>
              ))}
            </div>
